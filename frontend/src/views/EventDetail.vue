...
const closeAllPopups = () => {
  ...
  activeTab.value = 'scheme'; // Сбрасываем вкладку на "Схему"
...
const decrementTicket = (index) => {
  if (ticketCounts.value[index] > index0) {
    ticketCounts[index]--;
  }
};
...
const totalCost = computed(() => {
  return ticketCounts.value.reduce((sum, count, index) => {
    return sum + (count * (event.value.events_ticket[index]?.data?.price || 0));
  }, 0);
});
...
const submitOrder = () => {
  const selectedTickets = ticketCounts.value
    .map((count, index) => ({
      id_events_ticket: event.value.events_ticket[index]?.id_events_ticket,
      count,
    }))
...
const getImageUrl = (imgSrc) => {
  return imgSrc ? `https://api.ticket-now.ru/uploads/${imgSrc}` : '/assets/media/temp/event-detail.png';
};
  ...
  <main>
      ...
      <div class="popup__body">
        ...
        <div class="popup__switch_container">
          ...
          >
            <img
              class="shema__image"
              src="@/assets/media/temp/shema.png"
              alt=""
            />
          </div>
          <div
    ...
    >
      ...
      <div class="popup__body">
        ...
        <div class="form-group">
          <label for="email">Ваш email (туда будут высланы билеты)</label>
          <input
            id="email"
    ...
    >
      ...
      <div class="popup__body">
        ...
        <p class="text__def">
          Билет будет направлен на указанную вами электронную почту
        </p>
      </div>
    </div>
